    ...
    public static class TypeExtension
    {
        ...
        public static Type GetGenericType(this Type type, Type genericTypeDefinition)
        {
            if (!genericTypeDefinition.IsGenericTypeDefinition)
                throw new ArgumentException("GetGenericType参数genericTypeDefinition:[{0}]必须是泛型定义.".FormatArgs(genericTypeDefinition.Name));
        ...
        public static Type[] GetTypesSafely(this Assembly assembly)
        {
            ...
            {
                var msg = "程序集[{0}]加载类型发生错误:{1}".FormatArgs(assembly.GetName().Name, exc.GetDetails());
        ...
        /// <returns></returns>
        public static T GetCustomAttribute<T>(this PropertyDescriptor property) where T : Attribute
        {
            return (T)property.Attributes.OfType<Attribute>().FirstOrDefault(p => p is T);
        }
    }
}