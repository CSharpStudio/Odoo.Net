...
namespace System
{
    ...
    public static class StringExtension
    {
        ...
        /// <returns></returns>
        public static string ToUnderlineFormat(this string name)
        {
            if (name.IsNullOrEmpty() || !name.Any(p => char.IsLower(p)))
                return name;
            var chars = new List<char> { char.ToLower(name[0]) };
            for (int i = 1; i < name.Length; i++)
            {
                if (char.IsUpper(name[i]) && name[i - 1] != '_')
                    chars.Add('_');
